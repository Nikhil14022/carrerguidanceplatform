...
export default function ParentLayout({ children }: { children: React.ReactNode }) {
    ...
    const [isMinimized, setIsMinimized] = useState(() => {
        if (typeof window !== 'undefined') {
            return localStorage.getItem('parent-sidebar-minimized') === 'true';
        }
        return false;
    });
    ...
    const toggleMinimize = () => {
        setIsMinimized(prev => {
            const next = !prev;
            localStorage.setItem('parent-sidebar-minimized', String(next));
            return next;
        });
    };
        ...
        <div className="h-full flex flex-col justify-between">
            <div className="flex-1 flex flex-col">
                <div className={`p-6 ${isMinimized ? 'flex justify-center' : ''}`}>
                    ...
                    {!isMinimized && <p className="text-sm text-slate-500 mt-1">Parent Portal</p>}
                </div>
        ...
        <div className="flex h-screen bg-slate-950 overflow-hidden text-slate-300">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-0">
                <header className="relative z-50 bg-slate-950 border-b border-slate-800 p-4 flex items-center justify-between md:bg-transparent md:border-transparent md:justify-end md:px-8 md:pt-8 md:pb-0">
                    ...
                    <h1 className="font-bold text-lg text-slate-100 md:hidden ml-2 flex-1">Parent Portal</h1>
