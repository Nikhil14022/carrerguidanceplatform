...
export default function ParentProfilePage() {
    ...
    const userName = session?.user?.name || 'Parent User';
    const userEmail = session?.user?.email || 'N/A';
    const initial = userName.charAt(0).toUpperCase();

    return (
        <div className="max-w-4xl mx-auto py-8 space-y-8">
            <h1 className="text-3xl font-bold text-slate-200">Parent Profile</h1>
            ...
            <div className="bg-white/5 rounded-2xl shadow-sm border border-white/10 p-8">
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8 text-center md:text-left">
                    <div className="w-24 h-24 bg-gradient-to-br from-sky-500 to-blue-600 text-white rounded-full flex items-center justify-center text-4xl font-bold shrink-0 shadow-lg shadow-sky-500/25">
                        {initial}
                    </div>
                    <div>
                        <h2 className="text-2xl font-bold text-slate-100">{userName}</h2>
                        <p className="text-slate-500 mt-1">Observer & Supporter</p>
                        <div className="mt-3 flex flex-wrap gap-2">
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-sky-50 border border-sky-200 text-xs font-medium text-sky-700">
