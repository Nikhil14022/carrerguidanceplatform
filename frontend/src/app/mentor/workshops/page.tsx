...
export default function MentorWorkshopsPage() {
    ...
    const fetchWorkshops = async () => {
        try {
            const res = await fetch('/api/mentor/workshops');
            if (res.ok) {
                const data = await res.json();
                setWorkshops(data.workshops || []);
                setCurrentUserId(data.mentorId || '');
    ...
    const deleteWorkshop = async (id: string) => {
        ...
        setActionLoading(true);
        try {
            const res = await fetch(`/api/mentor/workshops/${id}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const updateEnrollmentStatus = async (enrollmentId: string, status: string) => {
        setActionLoading(true);
        try {
            const res = await fetch(`/api/mentor/workshops/enrollments/${enrollmentId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    if (loading) return <div className="flex justify-center py-20"><div className="w-10 h-10 border-4 border-orange-500/20 border-t-orange-500 rounded-full animate-spin" /></div>;
        ...
        <>
            <div className="space-y-6 animate-fade-in text-slate-200">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-slate-100">Mentor Workshops</h1>
                        <p className="text-slate-500 mt-1 text-sm">Schedule group sessions and manage your assigned clients.</p>
                    </div>
                    <button onClick={() => setShowCreate(true)} className="px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-bold uppercase tracking-widest shadow-sm">+ Schedule Workshop</button>
                </div>
                ...
                <div className="grid lg:grid-cols-[1fr_400px] gap-6">
                    ...
                    <div className="space-y-3">
                        {workshops.length === 0 ? (
                            <div className="bg-white/5 border rounded-xl p-12 text-center shadow-sm">
                                <p className="text-slate-500">You haven't scheduled any workshops yet.</p>
                            </div>
                        ) : workshops.map(w => {
                            ...
                            return (
                                <div key={w.id} onClick={() => setSelectedWorkshop(w)}
                                    className={`bg-white/5 border rounded-xl p-5 cursor-pointer transition-all shadow-sm hover:border-orange-400 hover:shadow-md ${selectedWorkshop?.id === w.id ? 'border-orange-500 ring-1 ring-orange-500' : 'border-white/10'}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            {w.createdBy !== currentUserId && <span className="ml-2 text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-bold">Admin Created</span>}
                                        </div>
                    ...
                    {/* Detail Panel */}
                    <div className="bg-white/5 border shadow-sm rounded-xl p-6 space-y-5 sticky top-4 max-h-[80vh] overflow-y-auto hidden lg:block">
                        {selectedWorkshop ? (
                            <>
                                <div className="flex justify-between items-start pb-4 border-b border-white/5">
                                    ...
                                    {selectedWorkshop.createdBy === currentUserId && (
                                ...
                                {selectedWorkshop.meetLink && (
                                    <div className="bg-orange-500/10 border border-orange-100 p-4 rounded-xl">
                                        <span className="text-[10px] text-orange-800 font-bold uppercase tracking-widest block mb-1">Meeting Link</span>
                                        <a href={selectedWorkshop.meetLink} target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:text-orange-800 text-sm font-medium hover:underline break-all">{selectedWorkshop.meetLink}</a>
                                    </div>
                                )}
                                ...
                                <div className="pt-4 border-t border-white/5">
                                    ...
                                    {selectedWorkshop.enrollments.length === 0 ? (
                                        <p className="text-xs text-slate-500 italic">No assigned clients enrolled yet.</p>
                                    ) : (
                                        <div className="space-y-3">
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4 lg:pl-64" style={{ zIndex: 99999 }} onClick={() => setShowCreate(false)}>
                    ...
                    <div className="relative z-10 w-full max-w-lg max-h-[90vh] flex flex-col bg-slate-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1 bg-white/5">
                            <div>
                                ...
                                <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. Master Interview Skills"
                                    className="w-full border border-white/20 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 focus:outline-none bg-white/5 shadow-sm" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="What will this workshop cover?" rows={4}
                                    className="w-full border border-white/20 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 focus:outline-none resize-none bg-white/5 shadow-sm" />
                            </div>
                        </div>
                        <div className="p-6 shrink-0 flex gap-3 border-t border-white/5 bg-white/5">
                            ...
                            <button onClick={createWorkshop} disabled={actionLoading || !form.title || !form.date || !form.description}
                                className="flex-1 py-3 rounded-xl text-xs font-bold uppercase tracking-widest bg-orange-600 text-white hover:bg-orange-700 disabled:opacity-50 transition-colors shadow-sm">{actionLoading ? 'Saving...' : 'Schedule'}</button>
                        </div>
                    </div>
                </div>,
                document.body
